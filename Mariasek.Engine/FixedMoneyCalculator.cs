...
{
	public class FixedMoneyCalculator : MoneyCalculatorBase
	{
		//Default constructor for XmlSerialize purposes
		[Preserve]
		public FixedMoneyCalculator()
		{
		}
		...
		public FixedMoneyCalculator(Game g, NumberFormatInfo nfi)
			: base(g)
		{
			_nfi = nfi;
		}

		public FixedMoneyCalculator(Game g, Bidding bidding, GameComputationResult res)
			: base(g, bidding, res)
		{
		}

		public FixedMoneyCalculator(Hra gameType, Barva? trump, int gameStartingPlayerIndex, Bidding bidding, IGameTypeValues values,
				...
				if ((_gameType & Hra.Hra) != 0)
				{
					...
					else
					{
						if (GameWon)
						{
							...
							if (QuietHundredWon)
							{
								GameMoneyWon = QuietHundredValue * _bidding.GameMultiplier;
							}
						}
						else
						{
